...
struct read *
SelectChunkRemain(struct read *rd, ushort chunkSize, ushort it, lint max, lint gnS, lint *nS, lint gnN, lint *nN,
                  int nt) {
    struct read *chunk;
    lint i;
    lint j;
...
void SelectChunk(struct read *chunk, const int nChunk, struct read *rd, ushort chunkSize, lint max, lint gnS, lint *nS,
    ...
    lint i, j, it;
    /*
     * This Function gets a read vector and allocate data by the size defined by nChunk.
     * Map it in Device (Graphic Card Memory)
     * Link the chunks */
    // Defining chunk
