...
//---------------------------------------------------------------------------
   ...
   d_Seq =    (char*)malloc(size[0]);
   d_Index =  ( int*)malloc(size[1]);
   d_start =  (lint*)malloc(size[4]);
   d_length = ( int*)malloc(size[2]);
   d_Freq =   ( int*)malloc(size[3]);
...
//************************************************
   ...
   ComputeFreqNew(d_Index, d_Freq, d_start, d_length, offset[2], fourk, nS);

   // cudaFree(rd);

   // if ( cudaMallocHost((void**)&rd->Freq, size[3]) != cudaSuccess)
      // printf("\n[Error 9] %s\n", cudaGetErrorString(cudaGetLastError()));

   rd->Freq = (int*)malloc(size[3]);

   // if ( cudaMemcpy(rd->Freq, d_Freq, size[3], cudaMemcpyDeviceToHost) != cudaSuccess)
      // printf("\n[Error 10] %s\n", cudaGetErrorString(cudaGetLastError()));
